...
@onready var weaponMenu = get_tree().get_first_node_in_group("menu_weapon_selection")
...
func _ready():
	connect("hovered_weapon", Callable(weaponMenu, "display_description"))
	connect("selected_weapon", Callable(weaponMenu, "start_game"))
